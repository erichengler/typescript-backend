...
app.post('/awesome/applicants', async (req, res) => {
    ...
    try {
        const result = await pool
            .query('INSERT INTO userinfo (name, age, email) VALUES ($1, $2, $3) RETURNING *', [name, age, email]);
        res.json(result.rows[0]);
    }
    catch (error) {
...
app.put('/awesome/applicants/:id', async (req, res) => {
    ...
    try {
        const result = await pool
            .query('UPDATE userinfo SET name = $2, age = $3, email = $4 WHERE id = $1 RETURNING *', [userId, name, age, email]);
...
app.delete('/awesome/applicants/:id', async (req, res) => {
    ...
    try {
        const result = await pool
            .query('DELETE FROM userinfo WHERE id = $1 RETURNING *', [userId]);
        if (result.rows.length === 0) {
            res.status(404).json({ error: 'User not found' });
        }
        else {
